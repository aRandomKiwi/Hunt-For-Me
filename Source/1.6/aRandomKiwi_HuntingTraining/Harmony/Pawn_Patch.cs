...
namespace aRandomKiwi.HFM
{
    internal class Pawn_Patch
    {
        ...
        {
            [HarmonyPostfix]
            public static void Listener(Pawn __instance, ref float __result)
            {
                    ...
                    if (comp != null)
                    {
                        ...
                        if (comp != null && comp.huntingPackMembers != null && comp.huntingPackMembers.Count > 0 && origComp.huntingArrivedToWaitingPoint)
                        {
                            ...
                            foreach (var m in comp.huntingPackMembers)
                            {
                                if (m == null)
                                    continue;
                                int cur = (int)m.TicksPerMoveCardinal;
                                if (ret == -1 || ret < cur)
                                    ret = cur;
                            }

                            if (master != null && master.TicksPerMoveCardinal > ret)
                                __result = (int)master.TicksPerMoveCardinal;
                            else
                                __result = ret;
                    ...
                    if (comp != null)
                    {
                        ...
                        if (comp != null && comp.huntingPackMembers != null && comp.huntingPackMembers.Count > 0 && origComp.huntingArrivedToWaitingPoint)
                        {
                            ...
                            foreach (var m in comp.huntingPackMembers)
                            {
                                if (m == null)
                                    continue;
                                int cur = (int)m.TicksPerMoveDiagonal;
                                if (ret == -1 || ret < cur)
                                    ret = cur;
                            }

                            if (master != null && master.TicksPerMoveDiagonal > ret)
                                __result = (int)master.TicksPerMoveDiagonal;
                            else
                                __result = ret;
